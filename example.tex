...
\outroright{\smash{\raisebox{1.33cm/2}{\qrcode[height=1.33cm]{https://www.3blue1brown.com/topics/neural-networks}}}\begin{tikzpicture}[remember picture,overlay]
    \node[above left,btdl@color@white,scale=.475] at (current page.south east) {\href{https://www.3blue1brown.com/topics/neural-networks}{Trainiertes Netzwerk zum Testen}};
\end{tikzpicture}}
...
\begin{block}{Linearkombination}
    \[w_{0}a_{0} + w_{1}a_{1} + \cdots + w_{n}a_{n}\onslide<4->{-b}\]
\end{block}
\end{column}
\begin{column}{0.5\linewidth}
\centering
...
$\eta:$ Lernrate\\
{\footnotesize\textit{Springende Werte verhindern}}
\[\vec{W_{neu}}=\vec{W}-\eta\nabla C(\vec{W})\]
\end{block}
...
\begin{frame}{Gradienten bestimmen}
\begin{layout-imageonly}
\begin{columns}[c]
...
\onslide<2->{
Gewichte verändern\\
{\scriptsize\textit{Proportional zu Neuronen aus vorheriger Schicht}}\\
}
\onslide<3->{
Neuronen verändern\\
{\scriptsize\textit{Gewichte aus vorheriger Schicht verändern}}
}
\end{block}
\end{column}\hfill
